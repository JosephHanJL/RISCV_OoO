...
for source_file in programs/*.{s,c}; do 
    ...
    if $quiet
    then 
        if diff ./output/$program.out ../project-3/project-3/correct_out/$program.out | grep -q "@@@"
        then
            same=false
        fi
    else
        if diff ./output/$program.out ../project-3/project-3/correct_out/$program.out | grep "@@@"
        then
            same=false
        fi
    fi
    ...
    grep "@@.*cycles" ../project-3/project-3/correct_out/$program.out
